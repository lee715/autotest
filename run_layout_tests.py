...
import time
import shutil
#用例目录 sdcard/webkit/layout_tests/
...
def main(options, args):
  ...
  """
  #设置APK的名称
  apk_name = "DumpRenderTree-release.apk"
  ...
  if not args:
    path = '/';
    #by zhangyingjie
    #push_testcase = os.system("adb push layout_tests /mnt/sdcard/webkit/layout_tests")
  elif args=="http":
    os.popen("adb shell cp /sdcard/webkit/http.txt /sdcard/webkit/layout_tests_list.txt")
  else :
    path = ' '.join(args);
    print path
    #by zhangyingjie
    #push_testcase = os.system("adb push layout_tests/%s /mnt/sdcard/webkit/layout_tests/%s" %(path,path))

  #测试前重新安装apk
  #os.popen("adb uninstall -r com.android.dumprendertree")
  #os.system("adb install -r %s" %apk_name)
  ...
  if options.refresh_test_list:
    ...
    os.popen(adb_cmd+" shell rm -r /sdcard/webkit/layout_tests_results")
    print os.popen("adb shell ls /sdcard/webkit/").read()
    os.popen(adb_cmd+" shell rm /sdcard/layout_tests_*.txt")
 ...
 #每个case 运行时间
  ...
  #pull 运行具体结果到PC上 显示百分比
  shell_cmd_str = adb_cmd + " pull " + "/sdcard/webkit/layout_tests_results" + " " + results_dir
  adb_output = subprocess.Popen(shell_cmd_str, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
  ...
  #os.system("adb shell rm -r /mnt/sdcard/webkit/layout_tests_results")
